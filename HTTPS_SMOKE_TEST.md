...
This guide validates end-to-end auth flow with HTTPS backend on https://localhost:3443 and React frontend over HTTPS.

Prereqs:
- Backend: env includes `HOST=0.0.0.0`, `PORT=3443`, `JWT_SECRET=dev_secret`, `HTTPS_ENABLE=true`, `HTTPS_KEY_PATH`, `HTTPS_CERT_PATH`
- Frontend: `.env.local` contains `REACT_APP_API_BASE_URL=https://localhost:3443`
...
- curl -i -k -X OPTIONS https://localhost:3443/auth/signup \
...
4) Signup
- curl -i -k -X POST https://localhost:3443/auth/signup \
  -H "Origin: https://localhost:3000" \
...
5) Login
- curl -i -k -X POST https://localhost:3443/auth/login \
  -H "Origin: https://localhost:3000" \
  -H "Content-Type: application/json" \
...
6) Frontend config
- In dsp_web_frontend/.env.local set:
  REACT_APP_API_BASE_URL=https://localhost:3443
- Restart npm start. Console should log:
  [API] Base URL resolved to: https://localhost:3443
...
- Backend binds 0.0.0.0; access via https://localhost:3443 from the same machine.
